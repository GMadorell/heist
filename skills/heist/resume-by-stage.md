...
| `safehouse` | `score.md` exists. Re-enter step 6. Worktree from step 2 should exist. Safehouse setup runs with re-entry check: run `git worktree list` and check for `<repo-name>-heist-<slug>`. If worktree exists, skip worktree add. Symlink verification and repair happen on re-entry: verify `.heist/<slug>/` symlink is present and resolves (re-create if missing/broken). If worktree doesn't exist, re-create from scratch. This matches step 6's idempotent setup. |
