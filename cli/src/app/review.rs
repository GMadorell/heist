...
pub fn select(
    ...
    let branch_recorded = state.branch.ok_or(SelectError::NoBranch)?;
    let branch = RefValue::try_from_raw(branch_recorded.as_ref())
    ...
    let language_types: Vec<_> = paths
        .iter()
        .map(|p| {
            let content = git.read_file_at(repo_root, &branch, p).unwrap_or(None);
