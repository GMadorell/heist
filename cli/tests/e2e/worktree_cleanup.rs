...
fn setup_repo_with_worktree(slug: &str) -> (TempDir, TempDir, std::path::PathBuf) {
    ...
    init_cmd.current_dir(main_repo).arg("state").arg("init").arg(slug);
    ...
    assert!(output.status.success(), "worktree add should succeed: {}", String::from_utf8_lossy(&output.stderr));
...
fn removes_merged_heist_owned_worktree() {
    ...
    assert!(output.status.success(), "cleanup should succeed, stderr: {}", String::from_utf8_lossy(&output.stderr));
    let stdout = String::from_utf8_lossy(&output.stdout);
    assert_eq!(stdout.trim(), "removed my-slug");

    assert!(!worktree_path.exists(), ".worktrees/my-slug should be removed");
