...
fn save_writes_state_file_visible_to_exists_and_load() {
    let _cwd = TempCwd::new();
    let repo = FileStateRepository;
    let state = State::new(&slug("foo"), fixed_date()).expect("valid slug");
    ...
    assert_eq!(repo.load(&slug("foo")).expect("load should succeed"), state);
}
...
fn load_missing_slug_is_missing() {
    let _cwd = TempCwd::new();
    let repo = FileStateRepository;
    assert!(matches!(repo.load(&slug("nope")), Err(StateError::Missing)));
}
...
fn save_then_load_roundtrips() {
    let _cwd = TempCwd::new();
    let repo = FileStateRepository;
    let mut state = State::new(&slug("foo"), fixed_date()).expect("valid slug");
...
fn list_slugs_returns_initialised_slugs_sorted() {
    ...
    repo.save(
        &slug("zeta"),
        &State::new(&slug("zeta"), fixed_date()).expect("valid slug"),
    )
    ...
    repo.save(
        &slug("alpha"),
        &State::new(&slug("alpha"), fixed_date()).expect("valid slug"),
    )
    .expect("save should succeed");
...
fn list_slugs_ignores_directories_without_a_state_file() {
    ...
    repo.save(
        &slug("foo"),
        &State::new(&slug("foo"), fixed_date()).expect("valid slug"),
    )
    .expect("save should succeed");
    FileHeistDirRepository
        .create(&slug("empty-dir"))
