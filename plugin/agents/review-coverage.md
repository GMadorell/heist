...
Run `heist validation resolve <path>` for how to run the test suite and, if available, a coverage command. If no coverage tooling is configured, fall back to manual inspection: for each new or changed function/branch in the diff, find the test(s) that exercise it and confirm they actually assert on the behavior (not just call the code path with no meaningful assertion).
