...
5. **Getaway**: before `gh pr create`, run `heist base <slug>` and read its `pr_base` line; pass it explicitly as `gh pr create --base <pr_base>` (rather than omitting `--base`). If `heist base <slug>`'s `stale` field prints `true`, note it in the PR body as a heads-up that the base may already be merging, with the remedy `gh pr edit <n> --base <main>` once confirmed. Push and open the PR. PR body: summary, risk label, auto-fixes applied, remaining `ask-user`/`no-op` findings.
