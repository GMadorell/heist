...
- Every assumption (stated or implied) not verifiable in the repo: check `heist validation resolve <absolute-path>` (`<absolute-path>` = the worktree root; in a monorepo subdir, pass the specific file paths you touched) and codebase directly, don't trust the blueprint's word.
