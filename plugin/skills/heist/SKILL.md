...
- If it contains a heist directory with a `state.json` whose `stage` isn't `done`: run `heist-cli resume <slug>` for the active slug, report its `stage`/`worktree` output to the human, then jump to the `next_step` it reports in `pipeline.md` (in this skill's directory).
