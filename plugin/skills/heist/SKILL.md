...
Precedence when tokens resolve differently from each other: an empty/unreadable file match always forces a halt, even if another token also resolves to a valid plan file in the same invocation — check every token before proceeding, don't stop at the first hit.
...
- Any token resolves to an existing file that is empty or unreadable: halt immediately with a diagnostic naming the file, regardless of whether other tokens matched a valid plan file. Do not create state, a slug, or a worktree — there is nothing to clean up yet.
- No token is empty/unreadable, and one or more resolve to existing, readable, non-empty files: plan-based heist, continue to the next subsection.
...
3. Ask the human via `AskUserQuestion`: list the source-set file(s) with absolute path and size, the total combined size, the mode, and the derived slug; ask whether to proceed as a plan-based heist.
   ...
   - Confirmed: read `pipeline.md` (in this skill's directory) and run it from step 1, carrying `<mode>`, "plan-based heist", the derived slug, the source set (absolute paths), and the prose through.
