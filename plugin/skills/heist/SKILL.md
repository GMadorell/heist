...
`--slug <slug>` is an optional leading token that pins the heist's slug instead of letting the Slugger pick one. When present, it's stripped and carried through step 2, which skips the Slugger and uses `<slug>` verbatim. `heat.md` emits it so a stacked piece's branch name (`heist/<slug>`) is deterministic and matches what a later piece names as its `--base`.
...
- `--slug <slug>` given: strip it off; carry it through step 2, which uses it verbatim and skips the Slugger.
- `--slug <slug>` omitted: step 2 spawns the Slugger as before.
