...
2. Relay loop — each Mastermind reply is a structured question, `SPLIT_PROPOSED`, or `INTERVIEW_COMPLETE`.
   ...
   - `SPLIT_PROPOSED`: show the human the piece list via `AskUserQuestion` with exactly three options: `accept`, `reject (continue unsplit)`, `redraw`.
     ...
     - Accept: relay `SPLIT_ACCEPTED`. The Mastermind writes `.heist/<slug>/heat.md`. Run it through `crit` the same way step 4 (human review) runs `crit` over `blueprint.md`: relay any comments to the Mastermind, ask it to apply them, repeat until the human leaves no comments. Once approved: run `heist worktree remove <slug>` (this already sets `stage done`; the parent branch has zero commits so the merge check passes). Tell the human the parent heist is done, and that `.heist/<slug>/heat.md` (which survives worktree teardown via the `.heist/<slug>` symlink) now holds one copy-pasteable `/heist:heist` prompt per piece. Sequential is the safe default: run a piece, merge it, then start the next. A stacked piece may be started before its base merges (to work through review latency), but only once its base piece's worktree exists, i.e. that base heist shows up in `heist list` past the `casing` stage; starting it earlier makes `heist worktree add --base heist/<base-slug>` fail because the base branch doesn't exist yet. Independent (unstacked) pieces can run concurrently in separate sessions.
