...
8. Getaway: run `heist base <slug>` first and pass its `pr_base` value to `gh pr create --base <pr_base>`. PR body: summary, note this heist ran in `light` mode (diff reviewed manually via crit, not the automated Cleaner review agents). If `heist base <slug>` prints a `warning: could not verify base's PR state` line to stderr, note it in the PR body as a heads-up that the base's PR state is unconfirmed, so a human should double check before assuming it's still open.
