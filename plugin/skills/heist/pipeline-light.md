...
5. Mergeable: ensure everything is committed, rebase onto `origin/<main>` (name from `heist validation resolve <absolute-path>`'s PR conventions). Resolve trivial conflicts yourself; surface real ones to the human and stop.
...
8. Getaway: push, `gh pr create`. PR body: summary, note this heist ran in `light` mode (diff reviewed manually via crit, not the automated Cleaner review agents).
